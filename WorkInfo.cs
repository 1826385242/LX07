...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by uploadTime desc) as num,workinfo.* from workinfo whrer activityId in @activityIds)";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workName,workPicture=@workPicture,uploadTime=@uploadTime," +
                    "userName=@userName,recommend=@recommend";
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workPicturn=@workPicture where workId=@workId";
        ...
        public int UpdateRecommend(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update from workinfo where workid=@id";
                return cn.Execute(sql, workinfo);
            }
        }
    }
